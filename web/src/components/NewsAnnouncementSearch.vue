...
const endDate = ref('')
const page = ref(1)
const dayPageOptions = [5, 10, 15, 20] as const
const selectedDaysPerPage = ref<number>(
  dayPageOptions.includes(props.daysPerPage as typeof dayPageOptions[number])
    ? props.daysPerPage
    : 5,
)
...
async function load() {
  ...
  try {
    result.value = await searchByDay(props.endpoint, {
      ...
      page: page.value,
      pageSize: selectedDaysPerPage.value,
  ...
  <section class="news-search" aria-label="新闻公告检索">
    ...
    <template v-else>
      ...
      <nav v-if="(result?.totalPages ?? 0) > 0" class="pagination" aria-label="检索结果分页">
        ...
        <div class="page-meta">
          <span>第 {{ page }} / {{ result?.totalPages }} 页 · 共 {{ result?.totalDays }} 天</span>
          <label class="page-size-selector">
            每页显示
            <select v-model.number="selectedDaysPerPage" @change="changeDaysPerPage">
              <option v-for="days in dayPageOptions" :key="days" :value="days">{{ days }} 天</option>
            </select>
          </label>
        </div>
      </nav>
...
@media(max-width:430px){.ai-button{padding-inline:10px}.cover-image{flex-basis:80px;width:80px;height:60px}.page-numbers{gap:2px;padding-inline:4px}.page-numbers button{width:30px;height:30px}.page-meta{flex-direction:column;gap:6px}}
</style>
