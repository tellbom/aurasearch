...
```text
Index API -> HTML cleaner -> DM aurasearch_desired_documents + one-row outbox/news_id
                                      |
                                  Outbox Worker
                              /                       \
...
1. 上游 `news_id` 是跨 DM、ES、Vespa 和埋点的稳定主键。
2. `index_version` 对同一 ID 严格递增；DM 期望状态、ES external version 和 Vespa condition 共同防乱序。
